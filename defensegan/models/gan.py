...
class DefenseGANBase(AbstractModel):
    ...
    def _loss(self):
        ...
        if self.mode == 'wgan':
            ...
            self.disc_train_op = tf.train.AdamOptimizer(
                learning_rate=2e-4,
                beta1=0.5
            ).minimize(self.discriminator_cost, var_list=self.discriminator_vars)
            # (disc_cost, var_list=self.discriminator_vars)

            self.clip_disc_weights = None
        elif self.mode == 'dragan':
            # G_loss = tf.losses.sigmoid_cross_entropy(tf.ones_like(self.disc_fake), logits=self.disc_fake)
            # D_loss_real = tf.losses.sigmoid_cross_entropy(tf.ones_like(self.disc_real), logits=self.disc_real)
            # D_loss_fake = tf.losses.sigmoid_cross_entropy(tf.zeros_like(self.disc_fake), logits=self.disc_fake)
            # D_loss = D_loss_real + D_loss_fake

            # self.generator_cost = G_loss
            # self.discriminator_cost = D_loss
            ...
            slopes = tf.sqrt(
                tf.reduce_sum(tf.square(gradients), reduction_indices=[1]))
            gradient_penalty = tf.reduce_mean((slopes - 1.) ** 2)
            self.discriminator_cost = self.discriminator_cost + \
                                      self.gradient_penalty_lambda * \
                                      gradient_penalty
    ...
    def reconstruct(
        ...
        def rec_body(i, *args):
            z_hats_recs = self.generator_fn(z_hat, is_training=False)
            # print('########### z hat rec type: {}'.format(type(z_hats_recs)))
            image_rec_loss = tf.reduce_mean(
                tf.square(z_hats_recs - images_tiled_rr),
                axis=axes)
            rec_loss = tf.reduce_sum(image_rec_loss)
        ...
        with tf.control_dependencies([init_z]):
            ...
            # print('####### final rec : {}'.format(final_recs))
            # print('####### final rec type : {}'.format(type(final_recs)))
            
            # # init = tf.global_variables_initializer()
            # # with  tf.Session() as sess:
            # #   init.run()
            # #   final_rec_nd = final_recs[-1].eval()
            # print('####### final rec nd : {}'.format(final_rec_nd))
            # print('####### final rec nd type : {}'.format(type(final_rec_nd)))


            online_rec = tf.stack(final_recs)
            print('####### online_rec : {}'.format(online_rec))
            print('####### online_rec : {}'.format(type(online_rec)))
...
class MnistDefenseGAN(DefenseGANBase):
    ...
    def generate_image_from_input(self, iteration):
        samples = self.sess.run(self.fixed_noise_samples)

...
def whitebox(gan, rec_data_path=None, batch_size=128, learning_rate=0.001,
    ...
    ####
    # print('##### num test: {}, num train: {}'.format(num_tests, num_train))
    # print('##### first image label: {}'.format(test_labels[0]))
    ####
    ...
    #     attack_iterations = 100
    #     attack_params = {'overshoot': 1,
    #                      'max_iter': attack_iterations,
    #                      'clip_min': min_val, 'clip_max': 1., 'nb_candidate': 4}
    ...
    if FLAGS.defense_type == 'defense_gan':
        ...
        my_img = plt.imread(args.input_address)
        ...
        feed_dict = {images_pl: my_img.reshape((1, 28, 28, 1))}
        mypred = tf.argmax(preds, axis=-1)
        myres = sess.run(mypred, feed_dict=feed_dict)
        print('###### my res: {}'.format(myres))

        z_init_val = tf.constant(np.random.randn(1 * gan.rec_rr, gan.latent_dim).astype(np.float32))
        rec = gan.reconstruct(tf.convert_to_tensor(my_img.reshape((1, 28, 28, 1)), np.float32), batch_size=1, back_prop=True, reconstructor_id=0, z_init_val=z_init_val)
        
        # with tf.Session() as sess:
        #   init = tf.group(tf.global_variables_initializer(),tf.local_variables_initializer())
        #   init.run()
        #   recnd = rec.eval()
        recnd = rec.eval(session= sess)
        print("########### recnd : {}".format(type(recnd)))
        print("########### recnd shape : {}".format(recnd.shape))

        imgs = Image.fromarray(float2int(recnd.reshape(28, 28)))
        imgs.save("my_rec1.png")

        # feed_dict2 = {images_pl: test_images[0].reshape((1, 28, 28, 1)), labels_pl: test_labels[0].reshape((1,10))}
        # myadvpred = tf.argmax(preds_adv, axis=-1)
        # myadv = sess.run(myadvpred, feed_dict=feed_dict2)
        # print('###### my adv: {}'.format(myadv))
        
        # print("label:")
        # print(test_labels[0])
        # print("shape: {}".format(labels_pl.shape))
        # print('##### image:')
        
        # plt.imsave('testimg.png', np.clip(test_images[0].reshape(28, 28), 0, 1), cmap='gray')
        ######
        return 0, 0, 0
...
def parse_args():
    ...
    parser.add_argument("--input_address", type=str,
                        help="address of input image")
